...
#define SYS_MAKEPIPE  ( 0x09 )
#define SYS_WRITEPIPE ( 0x10 )
#define SYS_READPIPE  ( 0x11 )
#define SYS_CLOSEPIPE ( 0x12 )
...
// makes the process waits for  a determined period of time 
extern void wait();

// get a process  pid 
extern pid_t get_pid();

/* creates a pipe  given two processes , p0 and p1  which uses the file descriptor 
fd[1] to write and fd[0] to read hence the pipe allows bidirectional communication */
extern int make_pipe(pid_t p0, pid_t p1);

// executing process writes the buffer  to the  pipe identified by  id  
extern void  write_to_pipe(int id, int buffer);


// executing process reads the buffer from the pipe indetified by id
extern int read_from_pipe(int id);

// close the pipe identified by id 
extern int _close_pipe(int id);
